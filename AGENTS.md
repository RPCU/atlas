...
- `palworld/` - Palworld dedicated game server, image `thijsvanloef/palworld-server-docker:latest`
  ...
  - `deploy.yaml` - Deployment (1 replica, Recreate, fsGroup 1000, 4-16Gi RAM, 2-4 CPU). Env vars from ConfigMap (`palworld-config`) via `envFrom`; `ADMIN_PASSWORD`/`SERVER_PASSWORD`/`DISCORD_WEBHOOK_URL` all sourced from the ESO Secret `palworld-secrets`. Single volume mount `/palworld` from PVC `palworld-data`. Plus a `playit` sidecar (`ghcr.io/playit-cloud/playit-agent`) whose `PLAYIT_SECRET_KEY` also comes from `palworld-secrets`; and a `exporter` sidecar (`docker.io/bostrt/palworld-exporter`, [palworldlol/palworld-exporter](https://github.com/palworldlol/palworld-exporter)) exposing Prometheus metrics on port 9877. It talks RCON to the game over localhost (`RCON_HOST=127.0.0.1`, `RCON_PORT=25575`, `RCON_PASSWORD` ← `palworld-secrets` key `ADMIN_PASSWORD` — RCON password == admin password in this image, RCON is enabled via `RCON_ENABLED=true` in `cm.yaml`) and reads save-file metrics from the `data` PVC mounted read-only at `/palworld` (`SAVE_DIRECTORY=/palworld`; the exporter recursively locates `Level.sav`)
  - `service.yaml` - ClusterIP service: ports 8212 TCP (REST API), 25575 TCP (RCON), 9877 TCP (exporter metrics). (Game/query UDP traffic is not on this Service — it's a NodePort elsewhere / playit tunnel)
  - `servicemonitor.yaml` - `ServiceMonitor` `palworld` (`monitoring.coreos.com/v1`) selecting the `palworld` Service's `metrics` (9877) endpoint. **MUST** carry label `release: kube-prometheus-stack` — the argus/Sveltos-pushed kube-prometheus-stack Prometheus on the production cluster uses `serviceMonitorSelectorNilUsesHelmValues: true`, so unlabeled ServiceMonitors are silently ignored. Scraped metrics (`palworld_*`) are `remote_write`n to central Mimir on mgmt (tagged `cluster="production"`). The matching **Grafana dashboard lives in the argus repo** (`infrastructure/grafana/dashboards/palworld-dashboard.yaml`) because Grafana is mgmt-only
...
| palworld-exporter | v1.3.1                 | `clusters/production/palworld/deploy.yaml`                                        |
