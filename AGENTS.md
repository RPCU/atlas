...
- `palworld/` - Palworld dedicated game server, image `thijsvanloef/palworld-server-docker:latest`
  ...
  - `deploy.yaml` - Deployment (1 replica, Recreate, fsGroup 1000, 4-16Gi RAM, 2-4 CPU). Env vars from ConfigMap (`palworld-config`) via `envFrom`; secrets (`ADMIN_PASSWORD`/`SERVER_PASSWORD`) from the manually-managed direct Secret `palworld-secrets` and `DISCORD_WEBHOOK_URL` from the ESO Secret `palworld-discord`. Single volume mount `/palworld` from PVC `palworld-data`. Plus a `playit` sidecar (`ghcr.io/playit-cloud/playit-agent`) whose `PLAYIT_SECRET_KEY` also comes from `palworld-secrets`
  ...
  - `secrets.yaml` - ESO `ExternalSecret` `palworld-discord` (`DISCORD_WEBHOOK_URL` ← Vault `secrets-production/palworld/config`) via the `vault-backend` ClusterSecretStore. **Populate this Vault path out of band.**
  - **Direct Secret `palworld-secrets`** (`ADMIN_PASSWORD`, `SERVER_PASSWORD`, `PLAYIT_SECRET_KEY`) — **NOT in Git, NOT via Vault/ESO**, managed manually/out of band. **Change default values before pushing!**
...
- `secrets-production/palworld/config` — palworld `DISCORD_WEBHOOK_URL` for the server's Discord lifecycle notifications (**populate out of band**)
